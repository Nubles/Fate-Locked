...
import { Sparkles, Map, Box, Copy, Shield, BookOpen, Footprints, Zap, Home, Store, Gamepad2, Skull, Package, Dna, ExternalLink } from 'lucide-react';
...
export const VoidReveal: React.FC<VoidRevealProps> = ({ itemName, itemType, itemImage, onComplete, isChaos = false }) => {
  const [phase, setPhase] = useState<Phase>(isChaos ? 'roulette' : 'imploding');
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-md overflow-hidden">
      ...
      {phase === 'reveal' && (
        <div className="relative z-[120] flex flex-col items-center animate-float-up">
          ...
          {/* The Card */}
          <div className="relative bg-gradient-to-b from-gray-900 to-black border-2 border-gray-700 p-8 rounded-xl shadow-2xl text-center min-w-[340px] max-w-md">
            {/* Rarity Star */}
            <div className={`absolute -top-6 left-1/2 -translate-x-1/2 bg-black border-2 ${theme.borderInner} p-3 rounded-full shadow-lg`}>
              {isChaos ? (
                  <Dna className="w-8 h-8 text-red-500 fill-current animate-pulse" />
              ) : (
